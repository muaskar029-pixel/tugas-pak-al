...
Panduan ini menggunakan PHP-FPM bawaan Arch Linux, bukan `php-legacy`, agar service dan path konfigurasi mengikuti paket PHP standar.

Install Apache, MariaDB, firewalld, PHP, PHP-FPM, dan modul GD:

```bash
sudo pacman -S --needed apache mariadb firewalld php php-fpm php-gd
```
...
Jika service PHP-FPM tidak ditemukan, cek nama unit PHP:
...
```bash
php -m | grep -E 'mysqli|pdo_mysql|gd|zip|mbstring'
```
...
```bash
grep -E '^listen' /etc/php/php-fpm.d/www.conf
```

Biasanya socket PHP-FPM berada di:
...
5. PHP dijalankan melalui PHP-FPM.
...
Server **Arteri** berhasil dideploy secara native menggunakan **Apache + PHP-FPM + MariaDB** di atas **Arch Linux linux-hardened**.
